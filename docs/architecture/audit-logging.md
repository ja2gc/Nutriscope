...
- pseudonymous patient code from the dedicated snapshot;
...
Patient identity and actor identity are separate DTO fields. The patient code answers which patient the event concerned without repeating the patient's name; the actor answers who performed it.

New events use the dedicated `patient_code_snapshot` column. Codes are random, immutable `NS-XXXX-XXXX` identifiers and are not derived from a database ID, UUID, date, or creation order. The former encrypted `patient_display_name_snapshot` column remains only for retained-row compatibility and is neither written nor returned by current audit flows. Neither snapshot is copied into `properties`, revisions, logs, metrics, URLs, exports, filters, sorting, or search. Backfill resolves the currently related patient when possible; unresolved historical events retain only the pseudonymous NCP reference.
...
The manual `php artisan audit:backfill-oversight --chunk=500` command classifies eligible legacy rows and backfills patient-code snapshots. It is chunked, deterministic, idempotent, unscheduled, and limited to null/wrong redesign metadata. It does not rewrite event payloads, legacy name snapshots, or revisions. Category/domain storage remains because retention and internal compatibility depend on it.
