...
### 3.2 Purchase-unit procurement (#4) — ✅ IMPLEMENTED 2026-06-14
**Decision B resolved: whole-pack overage carried as leftover stock** (no separate "buffer" line); it bakes into the base `qty` and is netted out next cycle by #2. Plan: `docs/superpowers/plans/2026-06-14-fs-purchase-unit-procurement.md`. Delivered: `purchase_qty`/`purchase_unit`/`purchase_price` columns on `shopping_list_items` + `purchase_order_items`; `ShoppingListController::generate` rounds `packs = ceil(net/basePerPurchase)` and writes both purchase + base fields (degrades to base-unit netting when `bpp ≤ 0`); SL→PO + manual PO create carry purchase fields; `ReceivingService` reads `purchase_qty × basePerPurchase` when present (falls back to `normalizeLine` for legacy/free-text); `ProcurementPackGenerator` AIR/Statement print purchase units (grand total unchanged by the invariant); procurement UI shows a "Buy" column. Invariant held: `purchase_qty × purchase_price == qty × unit_price`, so budget `cash_flow`/totals are unaffected. Verified by 6 feature tests in `FoodServiceOpsTest`; live browser click-through deferred (local MySQL was down at finish).
