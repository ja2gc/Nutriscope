...
| Seed behavior | Base seeder uses `activity()->withoutLogs`; no audit demo seeder exists | Keep base noise absent; owner declined a dedicated audit-event seeder; audit existing seeders for stale current-code values | Tasks 14–15 |
...
- [x] Task 8D: add `MenuCycleRevisionSerializer.php` plus read-only weekly menu version UI/tests; commit `feat: add menu audit history`.
- [x] Task 8E: add `ShoppingListRevisionSerializer.php` and `PurchaseOrderRevisionSerializer.php` plus shopping/PO/receiving version UI/tests; commit `feat: add procurement audit history`.
- [x] Task 8F: add `BudgetRevisionSerializer.php` plus contextual fiscal-year/ledger version UI/tests; commit `feat: add budget audit history`.
- [x] Serializers capture complete event-time operational structure, safe scalar values, child rows, units, totals, statuses, and stable operational references before deletion or after committed mutation as policy requires. They refuse clinical/patient-linked subjects and prohibited keys.
...
Task 8D followed red-green TDD. The initial backend gate failed because the menu-cycle serializer and revisions did not exist and simple menu changes lacked typed old/new values; the frontend gate failed because the menu-cycle historical component did not exist. Menu creation now stores an After-only complete weekly-plan snapshot. Structural day changes and schedule-date/activation-state changes store Before and After in the same fail-closed audited transaction as the canonical event, activation captures the complete plan and its event-time calculated totals, and deletion retains the final Before version after the live cycle is gone. Drawer-only scalar changes preserve typed old/new values, while active or PO-snapshotted structural edits remain rejected without an event or revision.

The serializer stores only public menu/recipe/catalog references and allow-listed schedule fields: dates, status, planned slots, meals, quantities, servings, population, event allocation, daily totals, total cost, and cost per head/day. It excludes actor/internal foreign keys, PO snapshots, raw model attributes, and unsupported nested data. Exact stored key sets, real calendar dates, bounded text/keys, nonnegative values, and safe enum tokens are revalidated before presentation. Daily population follows the same first-recorded headcount rule as the costing engine. The read-only UI uses the shared typed comparator to highlight added, changed, and removed fields, menu slots, and daily totals; history-type dispatch is an explicit switch ready for the remaining serializers. The final configured-MySQL gate passes 137 tests and 678 assertions across menu history, costing, cost freeze, workflow locks, PO snapshots, UUID input resolution, permissions, canonical events, operations audit, and the history framework. The final frontend gate passes 7 files and 24 tests across history, drawer interactions, and the UUID proxy. TypeScript, focused ESLint, full Pint, the production privacy/raw-serialization scan, and `git diff --check` pass. One transient Windows Vitest `spawn EPERM` occurred before test loading; an isolated retry and the fresh final gate both passed. Separate spec and code-quality reviews found no unresolved Task 8D issue.

Task 8E followed red-green TDD. The initial backend gate failed because shopping-list and purchase-order serializers and revisions did not exist. Additional red tests proved PO completion/archive, attachment upload, line-price correction, and no-op line-edit gaps; the broader gate identified stale child-event characterizations. Shopping-list create/generate now store After-only complete versions, list and line changes store one parent ShoppingList Before/After version, deletion retains Before after the live row is gone, and no-op line edits emit no event or revision. Purchase-order approval, order, ordinary update, receiving, vendor receipt, attachment upload/delete, price correction, completion, archive, and deletion now attach event-time PO-root versions inside the same fail-closed business transaction. Multiple corrected lines produce one parent Price Corrected event, while genuinely distinct receipt/upload/completion and budget side effects remain separate accountable events.

The shopping-list serializer allow-lists public list/item/catalog/supplier references, periods, aggregate population, quantities, units, prices, totals, scaling baselines, and vendor-lock state. The PO serializer allow-lists public PO/list/supplier/group/catalog/attachment references, status/lifecycle fields, safe attachment metadata, lines and totals, and aggregate operational PPA cost/output values. It excludes file paths, arbitrary planning/execution payloads, actor/internal foreign keys, model attributes, and prohibited clinical/report/AI fields; exact nested key sets are revalidated before presentation. The frontend adds explicit shopping-list/PO dispatch and read-only typed comparison views with added/changed/removed highlighting and no editing or raw serialization. The final configured-MySQL affected gate passes 143 tests and 796 assertions across procurement history, canonical events, shopping generation, receiving/completion, locks, attachment rollback, operations, serializers, and the history framework. The frontend gate passes 7 files and 24 tests; TypeScript, focused ESLint, full Pint, syntax checks, the production privacy/raw-serialization scan, and `git diff --check` pass. Spec and code-quality reviews found no unresolved Task 8E issue.

Task 8F followed red-green TDD. The initial backend gate failed because the Budget serializer and fiscal-year revisions did not exist and ledger events still used ledger children as subjects; the frontend gate failed because the Budget historical component did not exist. Fiscal-year creation now stores an After-only Budget version. Manual additions/deductions and idempotent PO deductions use Budget as the historical subject, retain Budget for its contextual trail, preserve PurchaseOrder as the PO-deduction context, and store complete Before/After ledger versions in the same fail-closed transaction. Locking the Budget row serializes concurrent append-only ledger snapshots without changing existing financial behavior or adding approval, reversal, or Admin mutation workflows.

The Budget serializer allow-lists fiscal year, opening allocation, legacy row-level per-head limit where present, ledger counts and typed totals, running balances, bounded reasons/references, safe actor names/public references, and PO numbers/public references. Ledger-row comparison keys are SHA-256 fingerprints of safe event-time fields rather than internal database IDs. Exact top-level, totals, and ledger key sets are revalidated; internal creator/PO IDs, deduction guards, arbitrary payloads, and prohibited clinical/report/AI fields are excluded. The UI uses the shared typed read-only comparator for added/changed ledger rows and balance changes. The configured-MySQL gate passes 69 tests and 426 assertions across Budget history, manual reason validation, ledger calculations/idempotency, Admin read-only behavior, PO completion/trails, operations, and the revision framework. The frontend gate passes 6 files and 25 tests; TypeScript, focused ESLint, Pint, syntax checks, the production privacy/raw-serialization scan, and `git diff --check` pass. Spec and code-quality reviews found no unresolved Task 8F issue.
...
- [x] Owner decision: defer the new cross-domain reason framework, endpoint enforcement, and client/modal work from this implementation. These remain future work and are not blockers for Tasks 10–17.
- [x] Existing budget manual-ledger input already requires and audits a reason. Task 10 preserves and verifies that behavior while adding the remaining approved fiscal-year, per-head/day, and PO-deduction presentation.

Future work may add bounded reasons for destructive and corrective Admin, clinical, RND library, FSS, and report workflows. It must still follow the original fail-closed validation, privacy, authorization, client coordination, and one-event requirements.
...
- [ ] Audit Task 8, Task 10, and Task 11 evidence against every original Task 12 requirement for FSS catalog, recipes, suppliers, menus, procurement, operations, and budget. Reuse verified coverage and add code only for genuine gaps, but do not mark Task 12 complete from analysis alone: every original coverage, writer, presenter, compatibility, and test checkbox below must be resolved with evidence.
...
- [x] Owner decision: skip the dedicated audit-event demo seeder. Do not add `AuditDemoSeeder.php` or synthetic audit scenarios.
- [x] Base seeding already suppresses audit logging and its zero-anonymous-noise contract remains covered. Existing application seeders, not audit-event seeders, remain subject to the name/staleness checks in Task 15.
...
- [ ] Re-audit existing seeders for stale values and outdated contracts beyond the completed first/last-name migration. Fix genuine current-code mismatches, rerun seeder idempotence/noise tests, and check reports or other consumers made stale by those values. Do not add a dedicated audit-event seeder.
...
| 14 | Owner-skipped dedicated audit-event seeder. Failure boundary is limited to base/current seeder noise or stale values, verified in Task 15. | `cd backend; php artisan test --compact tests/Unit/SeederIntegrityTest.php tests/Unit/FoodServiceDemoSeederSourceTest.php tests/Feature/PersonNameSeederTest.php` | Revert bounded current-seeder fixes; no synthetic audit rows exist. |
...
php artisan test --compact tests/Unit/SeederIntegrityTest.php tests/Unit/FoodServiceDemoSeederSourceTest.php tests/Feature/PersonNameSeederTest.php
