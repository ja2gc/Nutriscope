...
Password reset: sign-in-email lookup, generic forgot-password response, delivery only to the verified recovery email, independently verified shared recovery addresses, signed 15-minute broker token keyed to the sign-in email, single-use frontend reset URL, and all existing Sanctum tokens revoked after reset
