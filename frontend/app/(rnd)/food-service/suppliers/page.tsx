...
const inputCls =
  "w-full px-3 py-2 text-sm border border-zinc-200 rounded-lg text-zinc-900 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all";
      ...
      <div className="bg-white border border-zinc-200 rounded-2xl shadow-sm overflow-x-auto">
        {loading ? (
          ...
          <div className="py-16 text-center">
            <Truck className="h-8 w-8 text-zinc-300 mx-auto mb-3" />
            <p className="text-xs text-zinc-400 font-medium">No vendors yet. Add your first one.</p>
          </div>
        ) : (
            ...
            <thead className="bg-zinc-50 border-b border-zinc-100">
              <tr>
                {["Vendor", "Description", "Contact", "Payment Terms", "Actions"].map((h) => (
                  <th key={h} className="px-4 py-3 text-left text-[10px] font-bold text-zinc-500 uppercase tracking-wider whitespace-nowrap">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y divide-zinc-100">
              {filtered.map((s) => (
                <tr key={s.id} className="hover:bg-zinc-50/60 transition-colors">
                  <td className="px-4 py-3 font-semibold text-zinc-800">{s.name}</td>
                  <td className="px-4 py-3 text-zinc-600">{s.category || "—"}</td>
                  <td className="px-4 py-3 text-zinc-600">{s.contact || "—"}</td>
                  <td className="px-4 py-3 text-zinc-500">{s.payment_terms || "—"}</td>
                  <td className="px-4 py-3">
                    {deleteId === s.id ? (
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => setDeleteId(null)} className="text-[10px] font-bold text-zinc-500 hover:underline">No</button>
                      </div>
                    ) : (
                      <div className="flex items-center gap-1">
                        ...
                        <button onClick={() => setDeleteId(s.id)} title="Delete"
                          className="p-1.5 rounded-lg hover:bg-red-50 text-zinc-500 hover:text-red-600 transition-colors cursor-pointer">
