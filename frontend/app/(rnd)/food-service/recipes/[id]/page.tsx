...
interface InventoryItem {
  id: number;
  name: string;       // from food_item.name
  unit_price: number; // ₱/100g
  unit: string;
}
...
interface FSSRecipe {
  ...
  ingredients: Array<{
    ...
    quantity: number;
    unit: string;
    inventory: { id: number; name: string; unit_price: number; unit: string } | null;
  }>;
}
...
function calcCost(rows: IngredientRow[], servings: number): number {
  const total = rows.reduce((sum, row) => {
    if (!row.invItem || !row.quantity) return sum;
    const qty = parseFloat(row.quantity);
    if (isNaN(qty) || qty <= 0) return sum;
    return sum + (qty / 100) * row.invItem.unit_price;
  }, 0);
  return total;
}
        ...
        setIngredients(
          r.ingredients?.length
            ? r.ingredients.map((ing) => ({
                key: rowKey++,
                invItem: ing.inventory
                  ? { id: ing.inventory.id, name: ing.inventory.name, unit_price: ing.inventory.unit_price, unit: ing.inventory.unit }
                  : null,
                search: ing.inventory?.name ?? "",
                results: [],
                showDropdown: false,
