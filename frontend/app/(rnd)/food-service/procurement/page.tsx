    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-3">
          <button onClick={onBack} className="p-2 rounded-lg border border-zinc-200 hover:bg-zinc-50 text-zinc-500 cursor-pointer">
            ...
            <div className="flex flex-col gap-1">
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="w-36 px-3 py-2 text-sm border border-zinc-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 disabled:bg-zinc-50 disabled:text-zinc-400"
                />
                <Button
                  variant="ghost"
                  size="sm"
      ...
      {isSupplies && list.status === "draft" && (
        <div className="bg-white border border-zinc-200 rounded-2xl p-4 shadow-sm">
          <div className="text-[10px] font-extrabold text-zinc-500 uppercase tracking-wider mb-3">Add supply item</div>
          <div className="grid grid-cols-1 md:grid-cols-[1.4fr_90px_110px_150px_auto] gap-3 items-end">
            <div className="relative">
              <label className="block text-[10px] font-extrabold text-zinc-500 uppercase mb-1">Search supplies</label>
            ...
            <div>
              <label className="block text-[10px] font-extrabold text-zinc-500 uppercase mb-1">Qty</label>
      ...
      <div className="bg-white border border-zinc-200 rounded-2xl shadow-sm overflow-x-auto">
        <table className="w-full text-xs">
          ...
          <tbody className="divide-y divide-zinc-100">
            {list.items.length === 0 ? (
              <tr><td colSpan={7} className="px-3 py-8 text-center text-zinc-400">No items yet.</td></tr>
            ) : list.items.map((it) => (
                ...
                <td className="px-3 py-2">
                  {isSupplies && (
                    <button
                      ...
                      disabled={list.status === "converted"}
                      className="p-1.5 rounded-lg hover:bg-red-50 text-zinc-500 hover:text-red-600 cursor-pointer disabled:opacity-40"
          ...
          {list.items.length > 0 && (
            <tfoot className="bg-zinc-50 border-t border-zinc-200">
              <tr>
                <td colSpan={5} className="px-3 py-2.5 text-xs font-bold text-zinc-500 text-right uppercase tracking-wider">Total</td>
    ...
    return (
      <div className="space-y-5">
        <div className="flex items-center gap-2 text-xs font-semibold text-zinc-400">
          ...
          <span className="text-zinc-700">{group.supplier?.name ?? "Unassigned vendor"}</span>
        </div>
        <div className="flex items-center gap-3">
          <button onClick={() => setGroupId(null)} className="p-2 rounded-lg border border-zinc-200 hover:bg-zinc-50 text-zinc-500 cursor-pointer">
            <ChevronLeft className="h-4 w-4" />
          </button>
          <div>
      ...
      <div className="flex items-center gap-3">
        ...
        {po.lifecycle_status !== "open_execution" && po.actual_budget_per_head_per_day != null && (
          <div className="ml-auto flex flex-col items-end">
            <span className="text-[10px] font-extrabold text-zinc-500 uppercase tracking-wider">Actual budget / head / day</span>
            <span className="text-2xl font-extrabold text-emerald-700">{peso(num(po.actual_budget_per_head_per_day))}</span>
            <span className="text-[10px] text-zinc-400">final total ÷ total served population</span>
          </div>
        )}
      </div>
      <div className="bg-white border border-zinc-200 rounded-2xl shadow-sm overflow-x-auto">
          ...
          {tab === "supplies-lists" && (
            <div className="flex items-center gap-2">
              <input
                ...
                className="w-44 px-3 py-2 text-xs border border-zinc-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500"
              />
        ...
        {([
          ...
          <button key={k} onClick={() => setTab(k)}
            className={`px-5 py-3 text-sm font-semibold border-b-2 transition-colors cursor-pointer ${
              tab === k ? "border-emerald-600 text-emerald-700" : "border-transparent text-zinc-500 hover:text-zinc-800"
            }`}>
            {label}
          </button>
        ))}
          ...
          {loading ? (
                    ...
                    <tr key={l.id} className="hover:bg-zinc-50/60">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          <button onClick={() => setListDetail(l.id)} className="p-1.5 rounded-lg hover:bg-zinc-100 text-zinc-500 cursor-pointer" aria-label={`Open ${l.name}`} title="Open">
                            <Eye className="h-3.5 w-3.5" />
                          </button>
                          <button
                            onClick={() => { setEditingListId(l.id); setEditingListName(l.name); }}
                            className="p-1.5 rounded-lg text-zinc-500 hover:text-zinc-700 hover:bg-zinc-100 cursor-pointer"
                            aria-label={`Edit ${l.name}`} title="Edit"
                          >
                            <Pencil className="h-3.5 w-3.5" />
                          </button>
                          <button onClick={() => removeList(l.id)} className="p-1.5 rounded-lg hover:bg-red-50 text-zinc-500 hover:text-red-600 cursor-pointer" aria-label={`Delete ${l.name}`} title="Delete">
                            <Trash2 className="h-3.5 w-3.5" />
                          </button>
                        </div>
                      </td>
            ...
            procurementEvents.length === 0 ? (
              <div className="py-16 text-center">
                ...
                <p className="text-xs text-zinc-400 font-medium">No purchase orders yet. Convert a shopping list to generate a PO.</p>
              </div>
            ) : (
              <table className="w-full text-xs">
                <thead className="bg-zinc-50 border-b border-zinc-100">
                ...
                <tbody className="divide-y divide-zinc-100">
                  {procurementEvents.map(({ list, po }) => (
                    <tr key={list.id} className="hover:bg-zinc-50/60">
                      ...
                      <td className="px-4 py-3 text-zinc-500">{po ? po.lifecycle_status : "—"}</td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          <button
                            onClick={() => po ? setPoDetail(po.id) : setListDetail(list.id)}
                            className="p-1.5 rounded-lg hover:bg-zinc-100 text-zinc-500 cursor-pointer"
                            title="Open"
                          >
                            <Eye className="h-3.5 w-3.5" />
                          </button>
                          {po && po.lifecycle_status === "open_execution" && (
                            <button
                              ...
                              className="p-1.5 rounded-lg hover:bg-red-50 text-zinc-500 hover:text-red-600 cursor-pointer"
