...
function formatRiskLabel(score?: number | string | null) {
  if (score === null || score === undefined || score === "") {
    return { label: "Unscored", className: "bg-zinc-50 text-zinc-600 border-zinc-200" };
  }
  const n = Number(score);
  if (!Number.isFinite(n)) return { label: "Unscored", className: "bg-zinc-50 text-zinc-600 border-zinc-200" };
...
function formatStatus(status?: string | null) {
  switch ((status || "").toLowerCase()) {
    ...
    case "completed":  return { label: "Completed",  className: "bg-zinc-100 text-zinc-600 border-zinc-200" };
    case "discharged": return { label: "Discharged", className: "bg-orange-50 text-orange-700 border-orange-100" };
    default:           return { label: "Draft",      className: "bg-zinc-50 text-zinc-500 border-zinc-200" };
  }
}
...
function ConfirmBanner({
  ...
  return (
    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 p-4 bg-rose-50 border border-rose-200 rounded-xl">
      <AlertTriangle className="h-4 w-4 text-rose-500 shrink-0 mt-0.5 sm:mt-0" />
      <p className="text-xs text-rose-700 font-semibold flex-1">{message}</p>
      <div className="flex gap-2 shrink-0">
        <button
          onClick={onCancel}
          disabled={loading}
          className="px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider rounded-lg border border-zinc-200 text-zinc-600 hover:bg-zinc-50 transition-colors disabled:opacity-50"
        >
          Cancel
        </button>
        <button
          onClick={onConfirm}
          disabled={loading}
          className="px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider rounded-lg bg-rose-600 text-white hover:bg-rose-700 transition-colors disabled:opacity-50"
        >
    ...
    return (
      <div className="space-y-6 font-sans max-w-3xl mx-auto py-12 select-none">
        <div className="bg-red-50 border border-red-100 p-6 rounded-2xl text-center space-y-4">
          ...
          <h3 className="text-sm font-bold text-zinc-900 uppercase tracking-wider">Patient Profile Error</h3>
          <p className="text-xs text-zinc-500 max-w-sm mx-auto leading-relaxed">
            {error || "The requested patient record could not be found."}
          </p>
          <Link href="/ncp/patients" className="inline-flex px-4 py-2 bg-zinc-950 hover:bg-zinc-800 text-white font-semibold text-xs rounded-lg transition-all">
            Return to Directory
          </Link>
        </div>
      </div>
        ...
        <div className="flex flex-col xl:flex-row xl:items-start xl:justify-between gap-5">
          <div className="space-y-3">
            <div className="flex flex-wrap items-center gap-2.5">
              <h2 className="text-xl font-extrabold text-zinc-950 tracking-tight">{patient.name}</h2>
              <span className="text-xs font-mono px-2 py-0.5 bg-zinc-100 text-zinc-600 border border-zinc-200 rounded-lg">{systemId}</span>
            ...
            <div className="space-y-2">
              {!confirmDeletePatient ? (
                ...
                <div className="space-y-2">
                  ...
                  />
                  {patientDeleteError && (
                    <p className="text-xs text-rose-600 font-semibold">{patientDeleteError}</p>
                  )}
                </div>
              )}
            </div>
          ) : (
            <div className="flex items-center gap-2 px-3 py-2 bg-zinc-50 border border-zinc-200 rounded-lg w-fit">
      ...
      {activeTab === "overview" && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white border border-zinc-200 rounded-2xl overflow-hidden">
              ...
              <div className="p-5.5 grid grid-cols-1 sm:grid-cols-2 gap-y-4.5 gap-x-6 text-xs">
                <div className="space-y-1">
                  ...
                  <span className="text-zinc-800 font-semibold">{formatAbsoluteDate(patient.dob)}</span>
                </div>
                <div className="space-y-1">
                  <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider block">Admission Date</span>
                  <span className="text-zinc-800 font-semibold">{formatAbsoluteDate(patient.admission_date)}</span>
                </div>
                <div className="space-y-1">
            ...
            <div className="bg-white border border-zinc-200 rounded-2xl overflow-hidden">
              <div className="px-5 py-4 border-b border-zinc-100 bg-zinc-50">
                <h3 className="text-xs font-bold text-zinc-900 uppercase tracking-wider">NCP Entry Notes</h3>
              </div>
              <div className="p-5.5 space-y-4 text-xs text-zinc-600 leading-relaxed">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="p-4 bg-zinc-50 border border-zinc-200 rounded-xl">
                    <span className="text-[9px] font-extrabold text-zinc-400 uppercase tracking-wider block">Referring Physician</span>
                    <span className="mt-1 block font-semibold text-zinc-800">{patient.physician || "Unassigned"}</span>
                  </div>
                  <div className="p-4 bg-zinc-50 border border-zinc-200 rounded-xl">
                    <span className="text-[9px] font-extrabold text-zinc-400 uppercase tracking-wider block">Latest Assessment</span>
          ...
          <div className="space-y-6">
            <div className="bg-white border border-zinc-200 rounded-2xl p-6">
              <h3 className="text-xs font-extrabold text-zinc-900 uppercase tracking-wider mb-3">Current Cycle Snapshot</h3>
              {latestRecord ? (
                <div className="space-y-3.5 pt-2">
                  ...
                  <div className="text-xs text-zinc-600">
                    <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider block mb-1">Cycle ID</span>
                    <span className="font-mono font-semibold text-zinc-800">{formatCycleId(latestRecord.id)}</span>
                  </div>
                  <div className="text-xs text-zinc-600">
                    <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider block mb-1">Next Follow-up</span>
                    <span className="font-semibold text-zinc-800">{formatAbsoluteDate(latestMonitoring)}</span>
                  </div>
                </div>
              ) : (
                <p className="text-xs text-zinc-500 leading-relaxed">No NCP cycles have been started for this patient yet.</p>
              )}
            </div>
          </div>
        </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              <h3 className="text-xs font-extrabold text-zinc-700 uppercase tracking-wider">
                NCP Cycles <span className="font-mono text-zinc-400 normal-case ml-1">({records.length})</span>
              </h3>
              <p className="text-[10px] text-zinc-400 mt-0.5">Each cycle is an independent ADIME workflow for this patient.</p>
              <p className="text-[9px] text-zinc-400 mt-1 leading-relaxed max-w-md">
                ...
                return (
                  <div key={record.id} className="bg-white border border-zinc-200 rounded-2xl overflow-hidden">
                    ...
                    <div className="px-5 py-4 border-b border-zinc-100 flex items-center justify-between gap-4 bg-zinc-50">
                      <div className="space-y-1">
                        <div className="flex flex-wrap items-center gap-2">
                          <span className="text-sm font-extrabold text-zinc-950 tracking-tight">{cycleId}</span>
                      ...
                      <div className="flex items-center gap-4">
                        <div className="text-right text-xs text-zinc-500 hidden sm:block">
                          <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider block">Referring Physician</span>
                          <span className="font-semibold text-zinc-700">{patient.physician || "Unassigned"}</span>
                        </div>
                        {canDelete && !isConfirming ? (
                          ...
                          <span
                            ...
                            className="p-1.5 text-zinc-300 cursor-default select-none"
                          >
                    ...
                    <div className="p-5.5 grid grid-cols-1 lg:grid-cols-2 gap-5">
                      <div className="space-y-4">
                        <div className="p-4 bg-zinc-50 border border-zinc-200 rounded-xl">
                          <span className="text-[9px] font-extrabold text-zinc-400 uppercase tracking-wider block">Assessment Summary</span>
                          <p className="mt-1 text-xs text-zinc-700 leading-relaxed">{assessmentSummary}</p>
                        </div>
                        <div className="p-4 bg-zinc-50 border border-zinc-200 rounded-xl">
                          <span className="text-[9px] font-extrabold text-zinc-400 uppercase tracking-wider block">Diagnosis Summary</span>
                          <p className="mt-1 text-xs text-zinc-700 leading-relaxed">{diagnosisSummary}</p>
                        </div>
                      </div>
                      <div className="space-y-4">
                        <div className="p-4 bg-zinc-50 border border-zinc-200 rounded-xl">
                          <span className="text-[9px] font-extrabold text-zinc-400 uppercase tracking-wider block">Intervention Summary</span>
                          <p className="mt-1 text-xs text-zinc-700 leading-relaxed">{interventionSummary}</p>
                        </div>
                        <div className="p-4 bg-zinc-50 border border-zinc-200 rounded-xl">
                          <span className="text-[9px] font-extrabold text-zinc-400 uppercase tracking-wider block">Monitoring Summary</span>
                          <p className="mt-1 text-xs text-zinc-700 leading-relaxed">{monitoringSummary}</p>
                        </div>
                      </div>
                    </div>
