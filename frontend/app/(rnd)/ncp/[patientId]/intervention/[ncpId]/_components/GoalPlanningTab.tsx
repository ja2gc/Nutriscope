...
export default function GoalPlanningTab({
  ...
  return (
    <div className="space-y-5">
      <p className="text-[10px] text-warm-400">Links behavioral counseling goals to measurable nutrient targets.</p>
        ...
        {[['Energy', energy, 'kcal'], ['Protein', protein, 'g'], ['Carbs', carbs, 'g'], ['Fat', fat, 'g']].map(([label, val, unit]) => (
          <div key={label} className="bg-emerald-50 border border-emerald-200 p-3 rounded-xl text-center">
            ...
            <p className="text-lg font-extrabold font-mono text-warm-900 mt-1">
              {val || '—'}
              <span className="text-[9px] font-normal text-warm-500 ml-0.5">{unit}</span>
            </p>
          </div>
        ))}
      </div>
      ...
      {microRows.length > 0 && (
        <div className="space-y-2">
          <p className="text-[9px] font-bold text-warm-400 uppercase tracking-widest">Micronutrient Targets</p>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
      ...
      {goals ? (
        <div className="bg-white border border-warm-200 rounded-xl p-4 space-y-2">
          <p className="text-[9px] font-bold text-warm-400 uppercase tracking-widest">Behavioral Goals</p>
          <p className="text-sm text-warm-700 leading-relaxed whitespace-pre-wrap">{goals}</p>
        </div>
      ) : (
        <p className="text-xs text-warm-400 italic">No counseling goals set. Add them in the Counseling tab.</p>
      )}
    </div>
  );
}
