    ...
    <div className="bg-white border border-warm-200 rounded-2xl p-5 shadow-sm space-y-4">
      ...
      {plan.pes_statements.length > 0 && (
        <div className="space-y-1.5">
          <p className="text-xs font-bold text-warm-400 uppercase tracking-widest flex items-center gap-1.5">
            <Stethoscope className="h-3 w-3" /> Nutrition Diagnoses (PES)
          </p>
          <ul className="space-y-1">
            {plan.pes_statements.map((pes, i) => (
              <li key={i} className="text-xs text-warm-600 leading-relaxed flex gap-1.5">
      ...
      <p className="text-xs text-warm-400 pt-1 border-t border-warm-100">
