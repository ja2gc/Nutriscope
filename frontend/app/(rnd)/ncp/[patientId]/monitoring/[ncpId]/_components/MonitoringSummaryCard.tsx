  ...
  if (!summary?.has_data) {
    return (
      <div className="bg-white border border-warm-200 rounded-2xl p-5 shadow-sm">
        <h3 className="text-sm font-extrabold text-warm-700 uppercase tracking-wider mb-2">Visit Summary</h3>
        <p className="text-sm text-warm-400">Log a visit to see the evaluation summary. A second visit unlocks visit-to-visit trends.</p>
      </div>
    );
  }
    ...
    <div className="bg-white border border-warm-200 rounded-2xl p-5 shadow-sm space-y-4">
      ...
      {summary.changes.length > 0 && (
        <div className="space-y-1.5">
          <p className="text-xs font-bold text-warm-400 uppercase tracking-widest">What changed</p>
              ...
              return (
                <div key={c.metric} className="flex items-center justify-between px-2.5 py-1.5 rounded-lg border border-warm-100 bg-warm-50/60">
                  <div className="min-w-0">
                    <p className="text-xs font-semibold text-warm-700 truncate">{c.label}</p>
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    {c.delta != null && (
                      <span className="flex items-center gap-0.5 text-xs font-mono text-warm-500">
      ...
      {summary.intake.length > 0 && (
        <div className="space-y-1.5">
          <p className="text-xs font-bold text-warm-400 uppercase tracking-widest">Intake vs prescribed</p>
          <div className="space-y-1.5">
      ...
      <div className="pt-3 border-t border-warm-100 space-y-2">
        ...
        {aiNarrative && (
          <div className="p-3 bg-emerald-50 border border-emerald-100 rounded-xl">
            ...
            <button onClick={runAiReview} disabled={aiLoading}
              className="mt-2 text-xs font-bold text-emerald-600 hover:text-emerald-700 transition-colors cursor-pointer">
              Regenerate
            </button>
          </div>
        )}
