...
export default function EditRecipePage({
  ...
  const searchFoods = useCallback(async (query: string, rowIdx: number) => {
    ...
    setIngredients((prev) => prev.map((r, i) => i === rowIdx ? { ...r, searchResults: res.data.slice(0, 5), showDropdown: true } : r));
  }, []);
  ...
  if (!recipe && error) {
    return <div className="p-6 bg-red-50 border border-red-100 rounded-2xl text-sm text-red-700 font-bold">{error}</div>;
  }
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="bg-white border border-warm-200 rounded-2xl p-6 space-y-4 shadow-sm">
          <div className="flex items-center justify-between">
            ...
            <button type="button" onClick={addRow} className="flex items-center gap-1.5 px-3 py-1.5 border border-warm-300 rounded-lg text-warm-600 hover:bg-warm-50 cursor-pointer transition-colors text-xs font-bold uppercase tracking-wider">
                    ...
                    <div className="absolute top-full left-0 right-0 z-30 mt-1 bg-white border border-warm-200 rounded-xl shadow-lg overflow-hidden">
                      {row.searchResults.map((food) => (
                        ...
                        >
                          ...
                          <div className="text-xs text-warm-400">{food.calories} kcal · P {food.protein ?? 0}g · C {food.carbs ?? 0}g</div>
                        </button>
                      ))}
                    </div>
                  )}
