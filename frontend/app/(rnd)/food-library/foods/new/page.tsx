    ...
    <div className="space-y-6 font-sans max-w-2xl mx-auto">
      ...
      <div className="border-b border-zinc-200 pb-5 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-xs text-zinc-500 mt-1 select-none">Manually add a food item not found in USDA FoodData Central.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="bg-white border border-zinc-200 rounded-2xl p-6 space-y-4 shadow-sm">
          ...
          <p className="text-[10px] text-zinc-400">Used for allergen filtering in meal planning.</p>
            ...
            {COMMON_ALLERGENS.map((a) => (
              <button
                ...
                className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide border transition-all cursor-pointer ${
                  allergens.includes(a) ? "bg-amber-100 border-amber-300 text-amber-800" : "bg-zinc-50 border-zinc-200 text-zinc-500 hover:border-amber-200 hover:text-amber-700"
                }`}
              >
                {a}
              </button>
