    ...
    <div className="space-y-6 font-sans max-w-2xl mx-auto">
      ...
      <div className="border-b border-warm-200 pb-5 flex items-center gap-4">
        ...
        <div>
          ...
          {food?.usda_fdc_id && (
            <p className="text-[10px] font-mono text-warm-400 mt-0.5">USDA FDC ID: {food.usda_fdc_id} — micronutrients auto-populated on import</p>
          )}
        </div>
      </div>
            ...
            <div key={group}>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                {(keys as readonly string[]).map((key) => (
                  <div key={key}>
                    <label className="block text-[9px] font-bold text-warm-400 uppercase tracking-wider mb-1">
                      {key.replace(/_/g, ' ')} <span className="normal-case font-normal">({NUTRIENT_UNITS[key]})</span>
                    </label>
                    {food?.usda_fdc_id ? (
                      <p className="text-sm font-semibold text-warm-700 px-3 py-2 bg-warm-50 border border-warm-200 rounded-lg">
