...
export async function DELETE(
  _req: NextRequest,
  { params }: { params: Promise<{ ncpRecordId: string; mealPlanId: string; dayId: string; itemId: string }> }
) {
  const cookieStore = await cookies();
  const token = cookieStore.get("nutriscope_token")?.value;
  if (!token) return NextResponse.json({ message: "Unauthenticated." }, { status: 401 });

  const { ncpRecordId, mealPlanId, dayId, itemId } = await params;

  const laravelRes = await fetch(
    `${LARAVEL_API}/rnd/ncp-records/${ncpRecordId}/meal-plans/${mealPlanId}/days/${dayId}/items/${itemId}`,
    {
      method: "DELETE",
