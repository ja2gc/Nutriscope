...
export async function GET(
  ...
  const laravelRes = await fetch(`${LARAVEL_API}/rnd/ncp-records/${ncpRecordId}/screening-document`, {
    headers: {
  ...
  if (!laravelRes.ok) {
    ...
    return NextResponse.json(
      { message: data.message ?? "Failed to fetch screening document from backend." },
