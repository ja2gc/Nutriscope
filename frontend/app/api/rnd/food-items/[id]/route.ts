...
export async function GET(
  ...
  const laravelRes = await fetch(`${LARAVEL_API}/rnd/food-items/${id}`, {
...
export async function PUT(
  ...
  if (!laravelRes.ok) {
    return NextResponse.json({ message: data.message ?? "Failed to update food item.", errors: data.errors }, { status: laravelRes.status });
  }
