...
export default function BackupsPage() {
  ...
  const load = useCallback(async (quiet = false, requestedPage = page) => { if (!quiet) setLoading(true); try { const [initialBackups, automatic] = await Promise.all([listBackups(requestedPage), getBackupSchedules()]); let backups = initialBackups; if (backups.data.length === 0 && requestedPage > 1) { requestedPage -= 1; backups = await listBackups(requestedPage); setPage(requestedPage); } setData(backups); setSchedules(automatic); setError(null); } catch { setError("Backups could not be loaded. Check the connection and try again."); } finally { if (!quiet) setLoading(false); } }, [page]);
  ...
  const create = async () => { setCreating(true); setError(null); try { await createBackup(); setPage(1); setMessage("Backup queued. Status will update automatically."); await load(true, 1); } catch (cause) { setError(cause instanceof Error ? cause.message : "Backup could not be queued."); } finally { setCreating(false); } };
  ...
  return <div className="space-y-6 font-sans"><PageHeader crumbs={[["Admin", "/admin/dashboard"], ["Backup & Recovery"]]} title="Backup & Recovery" icon={<DatabaseBackup className="h-5 w-5 text-brand-green-600" />} subtitle="Manage restore points, automatic schedules, and staged whole-system recovery." actions={<Button className="min-h-11" loading={creating} disabled={hasActive || busyId !== null} onClick={() => void create()}><DatabaseBackup className="h-4 w-4" />Create backup now</Button>} />
    ...
    {loading && (!data || !schedules) ? <Card padded role="status" className="py-12 text-center"><RefreshCw className="mx-auto h-6 w-6 animate-spin text-brand-green-600" /><p className="mt-3 text-sm font-semibold text-warm-600">Loading backup status</p></Card> : data && schedules && <><BackupStatusSummary summary={data.summary} schedules={schedules} /><BackupScheduleSettings schedules={schedules} disabled={savingSchedules || hasActive} onChange={(input) => void saveSchedules(input)} /><BackupList backups={data.data} busyId={busyId} onDelete={setDeleting} onKeep={(backup) => void keep(backup)} onRecovery={setRecovering} onCancelRecovery={(backup) => void cancel(backup)} /><Pagination meta={data.meta} page={page} onPageChange={setPage} /></>}
