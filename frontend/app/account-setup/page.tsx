...
export default function AccountSetupPage() {
  ...
  if (initializing || !user) {
    return <main className="min-h-screen animate-pulse bg-warm-50" aria-busy="true" />;
  }

  return (
    <main className="flex min-h-screen items-center justify-center bg-warm-50 p-5 sm:p-8">
      <form
        onSubmit={finish}
        className="w-full max-w-lg space-y-5 rounded-3xl border border-warm-200 bg-white p-6 shadow-sm sm:p-8"
      >
        <div className="flex items-start gap-3 border-b border-warm-100 pb-5">
          <span className="rounded-xl border border-emerald-100 bg-emerald-50 p-2.5 text-emerald-700">
          ...
          <div>
            ...
            <p className="mt-2 text-sm leading-6 text-warm-500">
              Replace the temporary password and add a recovery email. No email verification code is needed now.
            </p>
          </div>
        </div>
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          <Button type="submit" loading={saving} fullWidth className="min-h-11">
            Save account setup
          </Button>
          <Button
            type="button"
            variant="secondary"
            onClick={() => void doLater()}
            disabled={saving}
            fullWidth
            className="min-h-11"
          >
        ...
        <p className="text-xs leading-5 text-warm-500">
          If deferred, this reminder stays visible until both items are completed in Profile settings.
        </p>
      </form>
    </main>
  );
