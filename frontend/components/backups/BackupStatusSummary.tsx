...
import type { BackupSchedulesDto, BackupSummaryDto } from "@/types/backup";
...
export function BackupStatusSummary({ summary, schedules }: { summary: BackupSummaryDto; schedules: BackupSchedulesDto }) {
  const healthy = summary.status === "healthy";
  const label = healthy ? "Healthy" : summary.status === "failed" ? "Failed" : "Attention needed";
  const automaticLabel = schedules.message ?? ([schedules.daily, schedules.weekly, schedules.monthly].some((item) => item.enabled) ? "Enabled" : "Automatic backups are disabled.");
    ...
    <Card padded>
      <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
        ...
        <dl className="grid flex-1 gap-3 sm:grid-cols-3">
          ...
          <div className="rounded-xl bg-warm-50 p-3"><dt className="flex items-center gap-2 text-xs font-bold uppercase tracking-wide text-warm-500"><CalendarClock className="h-4 w-4" />Automatic backups</dt><dd className="mt-1 text-sm font-semibold text-warm-900">{automaticLabel}</dd></div>
      ...
      <p className="mt-4 text-xs leading-relaxed text-warm-500">Recovery test: {date(summary.last_recovery_test_at)}</p>
    </Card>
  );
}
