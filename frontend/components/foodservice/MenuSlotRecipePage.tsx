...
type Draft = Pick<MenuSlotRecipe, "name" | "reference_servings" | "planned_servings" | "prep_notes"> & {
    ...
    <div className="mx-auto max-w-5xl space-y-5 pb-10">
      ...
      <header className="flex flex-col gap-3 border-b border-warm-200 pb-5 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-warm-600">{canEdit ? "Changes apply only to this menu slot. The original recipe stays unchanged." : "View the recipe scaled to this menu slot's planned servings."}</p>
        </div>
      ...
      <form onSubmit={save} className="space-y-5">
        <section className="grid grid-cols-1 gap-4 rounded-2xl border border-warm-200 bg-white p-4 shadow-sm sm:grid-cols-2 sm:p-5">
          ...
          <label>
            <span className="mb-1 block text-xs font-extrabold uppercase tracking-wider text-warm-500">Planned servings</span>
            <input required type="number" min={1} value={draft.planned_servings} readOnly={!canEdit} onChange={(event) => setDraft({ ...draft, planned_servings: Math.max(1, Number(event.target.value)) })} className="min-h-11 w-full rounded-lg border border-warm-200 px-3 text-base read-only:bg-warm-50 focus:outline-none focus:ring-2 focus:ring-emerald-500" />
          </label>
        </section>

        <section className="rounded-2xl border border-warm-200 bg-white p-4 shadow-sm sm:p-5">
          <div className="mb-3 flex items-end justify-between gap-3">
            <div><h2 className="font-extrabold text-warm-900">Ingredients</h2><p className="text-xs text-warm-500">Baseline quantities scale automatically to {draft.planned_servings} servings.</p></div>
          </div>
          <div className="space-y-3">
            {draft.ingredients.map((ingredient, index) => {
              const scaled = scaledIngredientQuantity(ingredient.quantity, draft.reference_servings, draft.planned_servings);
              return (
                <div key={ingredient.fs_item_id} className="grid grid-cols-1 gap-3 rounded-xl border border-warm-100 bg-warm-50/50 p-3 sm:grid-cols-[minmax(0,1fr)_8rem_7rem_7rem_auto] sm:items-end">
                  <div><span className="block text-xs font-bold uppercase tracking-wider text-warm-400">Ingredient</span><span className="mt-2 block min-h-8 text-sm font-bold text-warm-800">{ingredient.name}</span></div>
                  ...
                  <div><span className="block text-xs font-bold uppercase tracking-wider text-warm-400">Scaled qty</span><span className="mt-1 flex min-h-11 items-center font-mono text-sm font-bold text-emerald-700">{scaled.toFixed(2)} {ingredient.unit}</span></div>
        ...
        <section className="flex flex-col gap-3 rounded-2xl border border-warm-200 bg-warm-50 p-4 sm:flex-row sm:items-center sm:justify-between">
          <div><p className="text-xs font-extrabold uppercase tracking-wider text-warm-500">Current calculated cost</p><p className="text-xl font-extrabold text-emerald-700">{peso(data.total_cost)} <span className="text-sm font-semibold text-warm-500">· {peso(data.cost_per_head)} / serving</span></p><p className="text-xs text-warm-400">Cost refreshes after saving.</p></div>
