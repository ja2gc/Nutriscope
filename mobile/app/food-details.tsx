...
export default function FoodDetailsScreen() {
  ...
  return <ScrollView className="flex-1 bg-[#F4F7F5]" contentContainerStyle={{ padding: 16, paddingBottom: insets.bottom + 28 }}>
    <View className="rounded-[24px] bg-[#0B6B4B] p-5"><ChefHat color="#D1FAE5" size={22} /><Text className="mt-3 text-2xl font-extrabold text-white">{data.name}</Text><Text className="mt-1 text-sm text-emerald-100">{sourceLabel} · read-only for Food Service Staff</Text></View>
    <View className="mt-3 flex-row gap-3"><View className="flex-1 rounded-2xl border border-[#CBEADB] bg-[#EAF7F1] p-4"><Users color="#087F5B" size={19} /><Text className="mt-2 text-2xl font-extrabold text-[#087F5B]">{servings}</Text><Text className="text-xs text-[#4D7464]">{data.planned_servings == null ? 'Baseline servings' : 'Planned servings'}</Text></View><View className="flex-1 rounded-2xl border border-[#F3DEC1] bg-[#FFF7EA] p-4"><Coins color="#A65B13" size={19} /><Text className="mt-2 text-xl font-extrabold text-[#A65B13]">{data.cost_per_head == null ? '—' : `PHP ${Number(data.cost_per_head).toFixed(2)}`}</Text><Text className="text-xs text-[#8A653B]">Cost per head</Text></View></View>
    {data.planned_servings == null ? <View className="mt-3 rounded-2xl border border-amber-200 bg-amber-50 p-3"><Text className="text-sm text-amber-800">RND has not set a planned population for this slot. Baseline recipe quantities are shown.</Text></View> : null}
